...
public class AccountController {
		...
		public ResponseEntity<String> changePassword(
				...
				Principal principal) {
			logger.info("person {} send request to password change",
					principal.getName());
			...
			logger.info("person {} password succesfully changed",
		...
		public ResponseEntity<String> updatePersonalInfo(
				@Valid @ModelAttribute PersonalInfo info, BindingResult result,
				Principal principal) {
			if (result.hasErrors()) {
				return new ResponseEntity<>(getErrors(result),
						HttpStatus.BAD_REQUEST);
